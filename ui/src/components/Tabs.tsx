...
export default function Tabs({ filter, setFilter, items }: TabProps) {
    ...
    return (
        <ul className="flex flex-wrap text-xs font-medium text-center text-gray-500 bg-slate-100 w-fit px-1 py-1 rounded-xl cursor-pointer">
            {items.map((item, index) => (
                <li key={index} className="me-2">
                    <a
                        className={`inline-block px-3 py-1 rounded-lg
                            text-nowrap overflow-x-clip 
